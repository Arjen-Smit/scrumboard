...
abstract class BaseGroupQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'scrumboard', $modelName = 'ScrumBoard\\Group', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `name` FROM `group` WHERE `id` = :p0';
        try {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \ScrumBoard\UserGroupQuery A secondary query class using the current class as primary query
     */
    public function useUserGroupQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUserGroup($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'UserGroup', '\ScrumBoard\UserGroupQuery');
    }
