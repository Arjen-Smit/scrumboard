...
abstract class BaseUserGroupQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'scrumboard', $modelName = 'ScrumBoard\\UserGroup', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `user_id`, `group_id` FROM `user_group` WHERE `user_id` = :p0 AND `group_id` = :p1';
        try {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \ScrumBoard\GroupQuery A secondary query class using the current class as primary query
     */
    public function useGroupQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinGroup($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Group', '\ScrumBoard\GroupQuery');
    }
